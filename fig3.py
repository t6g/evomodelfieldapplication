...
c = contourf(y, x, log10(z), zm)
plot(cy, cx, 'ws', markersize=ms, markeredgecolor='w')
axis([0, 20, 50, 0])
ax.set_aspect('equal')
title('t = 10')
ylabel('Denitrifiers (log$\mathrm{_{10}}$ mM)')
xticks([])
yticks([])

text(-5, -10, 'Figure 3', fontsize='x-large', color='b')
...
axis([0, 20, 50, 0])
ax.set_aspect('equal')
ylabel('Iron reducers (log$\mathrm{_{10}}$ mM)')
xticks([])
yticks([])
...
axis([0, 20, 50, 0])
ax.set_aspect('equal')
ylabel('Sulfate reducers (log$\mathrm{_{10}}$ mM)')
xticks([])
yticks([])
